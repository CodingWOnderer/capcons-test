...
export const useEditorStore = create<EditorStore>()(
    immer((set, get) => ({
        ...
        initializeFromTemplate: (template: EditorTemplate) => {
            ...
            try {
                if (!template || !template.pages || template.pages.length === 0) {
                    throw new Error('Invalid template: Template must have at least one page');
                }
                ...
                set((state) => {
                    state.loading = false;
                    state.error = error instanceof Error ? error.message : 'Failed to initialize template';
                });
            }
        },
        ...
        setPageState: (pageId: string, data: Data) => {
            set((state) => {
                state.websitePagesState[pageId] = data;
            });
        },
        ...
        switchPage: (pageId: string) => {
            ...
            try {
                ...
                if (!page) {
                    throw new Error(`Page with ID ${pageId} not found in current template`);
                }
                ...
                set((state) => {
                    state.loading = false;
                    state.error = error instanceof Error ? error.message : 'Failed to switch page';
                });
            }
        },
        ...
        publishTemplate: (font?: string) => {
            try {
                ...
                set((state) => {
                    state.error = error instanceof Error ? error.message : 'Failed to publish template';
                });
                return null;
            }
        },
...
export const useCurrentPage = () => useEditorStore(state => state.currentPage);
export const useCurrentPageId = () => useEditorStore(state => state.currentPageId);
export const useCurrentTemplateId = () => useEditorStore(state => state.currentTemplateId);
export const useWebsitePagesState = () => useEditorStore(state => state.websitePagesState);
